...
const handleExport = () => {
  ...
  const header = ['Name', 'Email', 'Followed On'];
  const rows = props.followers.map((follower) => [
    ...
    toCsvValue(formatFollowedAt(follower.created_at || follower.followed_at))
  ].join(','));
      ...
      <div class="modal-panel">
        <header class="modal-header">
          ...
          <div class="modal-actions">
            ...
            <button type="button" class="btn btn-outline" :disabled="!hasFollowers" @click="handleExport">Export CSV</button>
          ...
          <table class="followers-table">
            ...
            <tbody>
              <tr v-for="follower in followers" :key="follower.id || follower.user_id">
                <td>{{ follower.name || 'Unknown' }}</td>
                <td>{{ follower.email || 'N/A' }}</td>
                <td>{{ formatFollowedAt(follower.created_at || follower.followed_at) }}</td>
              </tr>
            </tbody>
          </table>
        </section>
