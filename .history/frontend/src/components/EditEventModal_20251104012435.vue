    ...
    methods: {
        ...
        updateEventInStore(eventId, payload) {
            ...
            if (this.$store.state.allEvents) {
                ...
                if (eventIndex !== -1) {
                    this.$set(allEvents, eventIndex, updatedEvent);
                    this.$store.commit('setAllEvents', allEvents);
                }
            }

            // Update clubOwnedEvents in store
            if (this.$store.state.clubOwnedEvents) {
                const clubEvents = [...this.$store.state.clubOwnedEvents];
                const eventIndex = clubEvents.findIndex(event => event.id === eventId);
                if (eventIndex !== -1) {
                    this.$set(clubEvents, eventIndex, updatedEvent);
                    this.$store.commit('setClubOwnedEvents', clubEvents);
                }
            }
        },
        ...
        async handleSubmit() {
            ...
            try {
                ...
                // Update store directly with the new event data
                this.updateEventInStore(eventId, payload);
