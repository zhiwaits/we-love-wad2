...
const ownedEventSource = computed(() => {
  ...
  console.log('ClubCalendar - ownedEventSource:', owned);
...
onMounted(async () => {
  ...
  if (userId) {
    console.log('ClubCalendar - dispatching fetchClubOwnedEvents');
    await store.dispatch('fetchClubOwnedEvents', { force: true });
    console.log('ClubCalendar - clubOwnedEvents after dispatch:', store.state.clubOwnedEvents);
  }
});
...
const clubEvents = computed(() => {
  ...
  console.log('ClubCalendar - clubEvents computed, ownerId:', ownerId, 'currentUser:', currentUser.value);
  if (!Number.isFinite(ownerId)) return [];
  const events = (ownedEventSource.value || []).filter((event) => {
    ...
    console.log('ClubCalendar - checking event:', event.id, 'ownerId:', possibleOwner, 'matches:', matches);
    return matches;
  });
  console.log('ClubCalendar - filtered clubEvents:', events);
  return events;
});
