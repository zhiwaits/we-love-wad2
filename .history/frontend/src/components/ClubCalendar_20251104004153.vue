<template>
  <div class="club-calendar">
    ...
    <FullCalendar ref="calendarRef" :options="calendarOptions" :events="calendarEventsRef" />
...
onMounted(async () => {
  ...
  // Wait for calendar to be ready and add events
  await nextTick();
  if (calendarRef.value) {
    console.log('Calendar ref available, adding events');
...
const updateCalendarEvents = () => {
  ...
  // If calendar is mounted, add events directly
  if (calendarRef.value) {
    console.log('Adding events directly to calendar');
...
const calendarOptions = ref({
  ...
  events: [], // Start with empty events, we'll add them programmatically
  eventDidMount: (info) => {
    console.log('Event mounted on calendar:', info.event.title, info.event.start);
  }
});
