...
const emit = defineEmits(['close', 'remove-rsvp', 'remove-event-rsvps', 'refresh']);
...
const formatRsvpTimestamp = (value) => {
  if (!value) {
    return '—';
  }
...
const bulkBusy = (eventId) => props.busyKey === `bulk:${eventId}`;
...
const handleExport = (group) => {
  ...
  downloadCsv([header.join(','), ...rows], `${group.eventTitle || 'event'}-rsvps.csv`);
};
...
const handleRemoveAll = (group) => {
  if (!group?.eventId || !Array.isArray(group.rsvps)) {
    return;
  }
  emit('remove-event-rsvps', { eventId: group.eventId, rsvps: group.rsvps });
};
      ...
      <div class="modal-panel modal-panel--wide">
        <header class="modal-header">
          ...
          <div class="modal-actions">
            ...
            <button type="button" class="btn btn-outline" @click="handleClose">Close</button>
          </div>
        </header>
            ...
            <header class="rsvp-group__header">
              ...
              <div class="rsvp-group__actions">
                ...
                <button
                  ...
                  :disabled="group.rsvps.length === 0 || bulkBusy(group.eventId)"
                  @click="handleRemoveAll(group)"
                >
                  {{ bulkBusy(group.eventId) ? 'Clearing…' : 'Clear RSVPs' }}
                </button>
              </div>
            </header>
              ...
              <table class="rsvp-table">
                ...
                <tbody>
                  <tr v-for="rsvp in group.rsvps" :key="`${group.eventId}-${rsvp.user_id ?? rsvp.userId}`">
                    ...
                    <td>{{ rsvp.status || '—' }}</td>
                    <td>{{ formatRsvpTimestamp(rsvp.created_at) }}</td>
