...
export default {
  ...
  computed: {
    ...
    selectedFiltersCount() {
      ...
      return this.filters.selectedCategories.length + this.selectedTags.length + statusCount;
    },
  ...
  methods: {
    ...
    handleTagInputKeydown(event) {
      ...
      } else if (event.key === 'Tab' && this.tagSearchQuery.trim()) {
        event.preventDefault();
        this.handleTagSubmit();
    ...
    handleTagSubmit() {
      ...
      if (!this.isTagSelected(normalized)) {
        ...
        this.tagFeedback = 'Tag already selected.';
      }
    },
...
<template>
  <aside class="filter-panel">
      ...
      <div v-show="showTags" class="filter-section-content">
        <div class="tag-search">
          <input
            ...
            @input="tagFeedback = ''"
          />
          <button type="button" class="tag-add-btn" @click="handleTagSubmit" :disabled="!tagSearchQuery.trim()">
            Add
          </button>
        </div>
        ...
        <div v-if="tagFeedback" class="tag-feedback">{{ tagFeedback }}</div>
        ...
        <div v-if="selectedTags.length" class="selected-tag-list">
          ...
          >
            #{{ tag }}
            <button type="button" class="selected-tag-remove" @click="removeTag(tag)" aria-label="Remove tag">
              ×
            </button>
          </span>
        </div>
    ...
    <div v-if="selectedFiltersCount > 0" class="active-filters">
      ...
      <!-- Selected Tags -->
      <div v-if="selectedTags.length > 0" class="active-filter-group">
...
.tag-add-btn:not(:disabled):hover {
  background-color: var(--color-primary-hover, #005fcc);
  transform: translateY(-1px);
}
...
.selected-tag-chip {
  ...
  align-items: center;
  gap: var(--space-4, 4px);
  background-color: var(--color-bg-1, #f1f5f9);
