...
.carousel-wrapper {
  flex: 1;
  overflow: visible; /* Changed from hidden to visible to show peeking items */
  border-radius: var(--radius-lg);
  width: 100%;
  max-width: 1128px;
  margin: 0 auto;
  padding: 0 60px; /* Add padding for peeking effect */
  position: relative;
}
...
.events-carousel {
  ...
  margin-left: -60px; /* Offset for peeking */
  margin-right: -60px; /* Offset for peeking */
}

/* Peeking effect for partially visible items */
.events-carousel .event-card {
  transition: opacity 0.3s ease, transform 0.3s ease;
}

.events-carousel .event-card:nth-child(-n+2) {
  opacity: 0.6;
  transform: scale(0.95);
}

.events-carousel .event-card:nth-last-child(-n+2) {
  opacity: 0.6;
  transform: scale(0.95);
}
