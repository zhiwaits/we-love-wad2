...
const clubFollowers = computed(() => store.state.clubFollowers || []);
...
const clubStats = computed(() => {
  ...
  return {
    ...
    currentRSVPs: clubRSVPs.value.length || store.getters.clubRsvpCount || 0,
...
const handleRemoveAllRsvps = async ({ eventId, rsvps }) => {
  ...
  rsvpActionKey.value = key;
  try {
    const uniqueUserIds = Array.from(new Set(rsvps.map((rsvp) => rsvp.user_id || rsvp.userId)));
