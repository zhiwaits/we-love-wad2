...
onMounted(async () => {
  ...
  console.log('ClubCalendar onMounted - allEvents length:', allEvents.value.length);
  
  // Ensure all events are loaded
  if (allEvents.value.length === 0) {
    console.log('ClubCalendar - fetching all events');
    await store.dispatch('fetchAllEvents');
    console.log('ClubCalendar - after fetch, allEvents length:', allEvents.value.length);
  }
});
...
const clubEvents = computed(() => {
  ...
  console.log('ClubCalendar clubEvents - ownerId:', ownerId, 'currentUser:', currentUser.value);
  console.log('ClubCalendar clubEvents - allEvents:', allEvents.value);
  
  if (!Number.isFinite(ownerId)) {
    console.log('ClubCalendar clubEvents - invalid ownerId, returning empty array');
    return [];
  }
  ...
  const filtered = allEvents.value.filter((event) => {
    ...
    const matches = Number.isFinite(numeric) && numeric === ownerId;
    console.log('ClubCalendar clubEvents - checking event:', event.id, 'ownerId:', possibleOwner, 'matches:', matches);
    return matches;
  });
  
  console.log('ClubCalendar clubEvents - filtered events:', filtered);
  return filtered;
});
...
const calendarEvents = computed(() => {
  ...
  console.log('ClubCalendar calendarEvents - final events:', events);
  return events;
});
