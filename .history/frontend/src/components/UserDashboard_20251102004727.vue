...
const PEEK_WIDTH = 56; // Keep in sync with carousel wrapper padding for Netflix-style peek
...
const calculateCardWidth = (containerWidth) => {
  ...
  if (items <= 1) {
    return Math.min(MAX_CARD_WIDTH, Math.max(width, MOBILE_MIN_CARD_WIDTH));
  }
  return Math.max(MIN_CARD_WIDTH, Math.min(MAX_CARD_WIDTH, width));
};

const currentMinCardWidth = computed(() => (itemsPerView.value <= 1 ? MOBILE_MIN_CARD_WIDTH : MIN_CARD_WIDTH));
...
const upcomingCardStyle = computed(() => ({
  ...
  minWidth: `${currentMinCardWidth.value}px`,
...
const isUpcomingPeek = (idx) => idx < upcomingCarouselIndex.value || idx >= upcomingCarouselIndex.value + itemsPerView.value;
const isSavedPeek = (idx) => idx < savedCarouselIndex.value || idx >= savedCarouselIndex.value + itemsPerView.value;
    ...
    <div class="container">
      ...
      <section class="dashboard-section">
        ...
        <div v-else class="carousel-container">
          ...
          <div
            ...
            ref="upcomingCarouselWrapper"
            :class="{
              'is-start': upcomingCarouselIndex === 0,
              'is-end': upcomingCarouselIndex >= upcomingCarouselMaxIndex
            }"
          >
            <div class="events-carousel" :style="{ transform: upcomingCarouselTransform }">
              <div
                ...
                :key="event.id"
                :class="['event-card', { 'is-peek': isUpcomingPeek(idx) }]"
                :style="upcomingCardStyle"
                :tabindex="isUpcomingPeek(idx) ? -1 : 0"
                role="button"
      ...
      <section class="dashboard-section">
        ...
        <div v-else class="carousel-container">
          ...
          <div
            class="carousel-wrapper"
            ref="savedCarouselWrapper"
            :class="{
              'is-start': savedCarouselIndex === 0,
              'is-end': savedCarouselIndex >= savedCarouselMaxIndex
            }"
          >
            <div class="events-carousel" :style="{ transform: savedCarouselTransform }">
              <div
                ...
                :key="event.id"
                :class="['event-card', { 'is-peek': isSavedPeek(idx) }]"
                :style="savedCardStyle"
                :tabindex="isSavedPeek(idx) ? -1 : 0"
                role="button"
                @click="openEventModal(event)"
                @keyup.enter.prevent="openEventModal(event)"
                @keyup.space.prevent="openEventModal(event)"
...
.carousel-wrapper.is-start::before {
  opacity: 0;
}

.carousel-wrapper.is-end::after {
  opacity: 0;
}
