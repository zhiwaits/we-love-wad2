      ...
      <div v-if="calendarEventsRef.length > 0">
        ...
        <ul>
          <li v-for="event in calendarEventsRef" :key="event.id">
            {{ event.title }} - {{ event.start }} to {{ event.end || 'No end time' }} ({{ event.extendedProps.eventStatus }})
          </li>
        </ul>
      </div>
    </div>
...
const updateCalendarEvents = () => {
  ...
  clubEvents.value.forEach(event => {
    console.log('Processing event:', event.id, event.title, event.date, event.datetime, event.enddatetime);
    // Use datetime if available, otherwise use date
    const dateToUse = event.datetime || event.date;
    const eventDate = new Date(dateToUse);
    console.log('Using start date:', dateToUse, 'parsed as:', eventDate, 'is valid date:', !isNaN(eventDate.getTime()));
    ...
    if (event.enddatetime) {
      ...
      console.log('Using provided end date:', event.enddatetime, 'parsed as:', eventEndDate);
    } else {
      // No end time specified, set to end of day
      eventEndDate = new Date(eventDate);
      eventEndDate.setHours(23, 59, 59, 999); // End of day
      console.log('No end time provided, setting to end of day:', eventEndDate);
    }

    // Validate end date
    if (isNaN(eventEndDate.getTime())) {
      console.error('Invalid end date for event:', event.id, event.enddatetime);
      eventEndDate = null; // Don't set end date if invalid
    }
    
    const isUpcoming = eventDate > now;
    console.log('Event start:', eventDate, 'end:', eventEndDate, 'isUpcoming:', isUpcoming, 'now:', now);
  ...
  calendarEventsRef.value = [...events]; // Update the ref
};
