...
const handleLocationSelected = (locationData) => {
	if (!useMapLocation.value) return;
	const lat = Number(locationData.latitude);
	const lng = Number(locationData.longitude);
	form.value.latitude = Number.isFinite(lat) ? lat : form.value.latitude;
	form.value.longitude = Number.isFinite(lng) ? lng : form.value.longitude;
};
...
watch(useMapLocation, async (enabled) => {
	if (!enabled) {
		...
		if (form.value.latitude == null || !Number.isFinite(Number(form.value.latitude))) {
			form.value.latitude = DEFAULT_LAT;
		}
		if (form.value.longitude == null || !Number.isFinite(Number(form.value.longitude))) {
			form.value.longitude = DEFAULT_LNG;
		}
		await nextTick();
	}
});
				...
				<form class="form" @submit.prevent="handleSubmit">
					...
					<div class="map-toggle">
						<label class="map-toggle__label">
							<input type="checkbox" v-model="useMapLocation" />
							Attach Google Maps location
						</label>
						<p class="field-hint">Optional: share precise coordinates so attendees can view the event on Google Maps.</p>
					</div>
					...
					<LocationPicker
						...
						:initial-lat="form.latitude ?? DEFAULT_LAT"
						:initial-lng="form.longitude ?? DEFAULT_LNG"
