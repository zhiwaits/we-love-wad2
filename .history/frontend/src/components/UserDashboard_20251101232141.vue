...
const upcomingCarouselTransform = computed(() => {
  const translateX = -upcomingCarouselIndex.value * (100 / 3); // Move by 1/3 of container width
  return `translateX(${translateX}%)`;
});

const savedCarouselTransform = computed(() => {
  const translateX = -savedCarouselIndex.value * (100 / 3); // Move by 1/3 of container width
  return `translateX(${translateX}%)`;
});
</script>
