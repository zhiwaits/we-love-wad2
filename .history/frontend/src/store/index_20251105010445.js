...
const parseDateValue = (value) => {
  ...
  if (typeof value === 'string') {
    ...
    if (trimmed.includes(' ')) {
      ...
      const getRandomWeight = (event) => {
        if (!event || event.id == null) return 0;
  ...
  getters: {
    ...
    filteredClubEvents: (state, getters, rootState) => {
      ...
      const clubEvents = sourceEvents.filter(event => getNumericOwnerId(event) === ownerId);
      ...
      if (!noFiltersActive) {
        ...
        if (state.clubEventFilters.venueFilter !== 'all') {
          filtered = filtered.filter(event => event.venue === state.clubEventFilters.venueFilter);
        }
        ...
        if (state.clubEventFilters.locationQuery) {
          ...
          filtered = filtered.filter(event => {
            ...
            const venue = (event.venue || '').toString().toLowerCase();
            return location.includes(locQuery) || venue.includes(locQuery);
          });
        }
      ...
      const option = state.clubEventFilters?.sortOption || 'newest';
      switch (option) {
        case 'earliest-date':
        case 'earliest_date':
        case 'earliestDate':
  ...
  mutations: {
    ...
    setClubOwnedEvents(state, events) {
      ...
      if (sortOption === 'random') {
        state.clubRandomSortMap = generateRandomSortMap(sourceEvents);
      } else {
        state.clubRandomSortMap = pruneRandomSortMap(state.clubRandomSortMap, sourceEvents);
      }
    },
