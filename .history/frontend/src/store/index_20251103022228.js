...
export default createStore({
  ...
  async fetchAvailableTags({ state, commit }, { force = false } = {}) {
    ...
    async fetchClubRSVPs({ commit, state, dispatch, rootGetters }, clubId) {
      ...
      try {
        // Try to use the owner endpoint first
        const { getRsvpsForEventsByOwner } = await import('../services/rsvpService');
        try {
          const response = await getRsvpsForEventsByOwner(ownerId);
          const rsvps = Array.isArray(response.data) ? response.data : [];
          
          // If the owner endpoint returns data, use it
          if (rsvps.length > 0) {
            commit('SET_CLUB_RSVPS', rsvps);
            return;
          }
        } catch (ownerError) {
          console.warn('Owner endpoint failed, falling back to per-event fetching:', ownerError.message);
        }

        // Fallback to fetching per event if owner endpoint doesn't work
        if (!Array.isArray(state.clubOwnedEvents) || state.clubOwnedEvents.length === 0) {
