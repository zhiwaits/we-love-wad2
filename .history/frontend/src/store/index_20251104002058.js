...
export default createStore({
  ...
  async fetchAvailableTags({ state, commit }, { force = false } = {}) {
    ...
    async fetchClubOwnedEvents({ state, commit, rootGetters }, { force = false } = {}) {
      ...
      console.log('fetchClubOwnedEvents - currentUser:', currentUser, 'role:', role, 'numericOwnerId:', numericOwnerId);

      if (role !== 'club' || !Number.isFinite(numericOwnerId)) {
        console.log('fetchClubOwnedEvents - user is not a club or no valid ID');
        if (force) {
      ...
      try {
        ...
        console.log('fetchClubOwnedEvents - total events fetched:', aggregatedEvents.length);
        console.log('fetchClubOwnedEvents - owned events found:', ownedEvents.length);
        console.log('fetchClubOwnedEvents - sample owned event:', ownedEvents[0]);
