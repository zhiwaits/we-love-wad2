...
const actions = {
   ...
   * Register a new user
   */
  async register({ commit }, { email, password, name, role, club_description, club_category_id, imageBase64, imageOriginalName }) {
    commit('SET_LOADING', true);
    commit('CLEAR_ERROR');
    
    try {
  const response = await authService.register(email, password, name, role, { club_description, club_category_id, imageBase64, imageOriginalName });
