...
exports.createClubProfile = async (req, res) => {
  try {
    const { username, email, password, club_description, club_category_id, imageBase64, imageOriginalName } = req.body;
    ...
    if (imageBase64 && imageOriginalName) {
      const uploadDir = path.resolve(__dirname, '../upload/club');
      try { fs.mkdirSync(uploadDir, { recursive: true }); } catch {}
      const ext = path.extname(imageOriginalName) || '.png';
      const safeBase = path.basename(imageOriginalName, ext).replace(/[^a-z0-9_-]/gi, '_').toLowerCase();
    ...
    const result = await pool.query(
      ...
      [username, email, passwordHash, club_description, club_category_id, storedImageUrl]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
