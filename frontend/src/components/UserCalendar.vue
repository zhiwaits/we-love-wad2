          ...
          <div class="modal-content">
            ...
            <div class="event-details">
              <span class="event-badge" :class="selectedEvent.extendedProps.eventType">
                {{ selectedEvent.extendedProps.eventType === 'rsvp' ? 'RSVP\'d' : 'Saved' }}
              </span>
...
const calendarEvents = computed(() => {
  ...
  // Add RSVP'd events (primary events - blue)
  allEvents.value.forEach(event => {
    if (userRSVPs.value.includes(event.id)) {
      events.push({
        id: `rsvp-${event.id}`,
        title: event.title,
        start: event.date,
  ...
  // Add saved events (secondary events - orange)
  allEvents.value.forEach(event => {
    // Only add if not already RSVP'd (avoid duplicates)
